...
function theme_options_add_page() {
	add_theme_page( __( 'テーマオプション', 'colorful' ), __( 'テーマオプション', 'colorful' ), 'edit_theme_options', 'theme_options', 'theme_options_do_page' );
	add_theme_page( __( 'プログラムの追加', 'colorful' ), __( 'プログラムの追加', 'colorful' ), 'edit_theme_options', 'custom_css', 'custom_css_do_page' );
}
...
?>
<div class="wrap">
<h2><?php _e( 'テーマオプション サイト全体設定', 'colorful' ); ?></h2>
<form method="post" action="options.php"> 
<?php settings_fields( 'entire_options' ); do_settings_sections( 'entire_options' ); ?>

<div id="tabs">
  <ul>
    <li><a href="#tab-content1"><?php _e( 'ブログ機能設定', 'colorful' ); ?></a></li>
  </ul>

<div id="tab-content1">
  <?php _e( '<p>ブログ利用も行いたい時に活用できる機能です。<br>
  ...
  動画マニュアルを参照の上、操作をお願いいたします。</p>', 'colorful' ); ?>

  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '背景設定', 'colorful' ); ?></h3>

    <h4 class="theme_option_headline_sub"><?php _e( '背景色', 'colorful' ); ?></h4>
    ...
    <h4 class="theme_option_headline_sub"><?php _e( '背景画像URL', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <input type="text" id="media1" name="entire_options[bg_image]" value="<?= $options['bg_image']; ?>" size="50">
      <button type="button" class="select-file button" data-input="media1"><?php _e( 'ファイルを選択', 'colorful' ); ?></button>
    </div>

    <h4 class="theme_option_headline_sub"><?php _e( '背景画像繰り返し', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
        $repeat_array = array (
    ...
    <h4 class="theme_option_headline_sub"><?php _e( '背景画像固定', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
    ...
    <h4 class="theme_option_headline_sub"><?php _e( '背景画像の配置', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '記事背景設定', 'colorful' ); ?></h3>
    ...
    <h4 class="theme_option_headline_sub"><?php _e( '影の設定', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
        $shadow_array = array (
          'on' => 'ON',
    ...
    <div id="shadow_color"<?php if($options['shadow_color']!='on') : ?> style="display:none"<?php endif; ?>>
      <h4 class="theme_option_headline_sub"><?php _e( '影の色', 'colorful' ); ?></h4>
      <div class="theme_option_input">
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( 'サイドバー背景設定', 'colorful' ); ?></h3>

    <h4 class="theme_option_headline_sub"><?php _e( '文字色', 'colorful' ); ?></h4>
    <div class="theme_option_input">
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '応用設定', 'colorful' ); ?></h3>

    <h4 class="theme_option_headline_sub"><?php _e( 'スマホ最適化(レスポンシブ対応)', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
        $sp_array = array (
          'on' => 'ON',
    ...
    <h4 class="theme_option_headline_sub"><?php _e( 'ページ幅設定', 'colorful' ); ?></h4>
      <input type="number" name="entire_options[pwidth]" value="<?= $options['pwidth']; ?>" size="50" step="1">
    <div class="theme_option_input">
    </div>

    <h4 class="theme_option_headline_sub"><?php _e( 'Webフォント設定', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
        $webfont_array = array (
          'on' => 'ON',
        ...
        $checked = '';
        $webfont = $options['webfonts'];
        foreach( $webfont_array as $value => $label ) :
      ?>
      <label>
        <input type="radio" name="entire_options[webfonts]" class="webfont" value="<?= $value; ?>"<?= ($webfont == $value) ? ' checked="checked"': ''; ?>><?= $label; ?>
      </label>　
      <?php endforeach; ?>
    </div>

    <h4 class="theme_option_headline_sub"><?php _e( 'フォント設定', 'colorful' ); ?></h4>
    <div id="font_family" class="theme_option_input"<?php if($options['webfonts']!='off') : ?> style="display:none"<?php endif; ?>>
    ...
    <div id="font_family_web" class="theme_option_input"<?php if($options['webfonts']=='off') : ?> style="display:none"<?php endif; ?>>
      <select name="entire_options[font_family_web]">
      <?php
        $font_array = array (
          '7' => 'Noto Sans JP',
          '8' => 'Noto Serif JP',
        );
        $font_family_web = $options['font_family_web'];
        foreach( $font_array as $value => $label ) :
      ?>
      <option value="<?= $value; ?>"<?= ($font_family_web == $value) ? ' selected="selected"': ''; ?>><?= $label; ?></option>
      <?php endforeach; ?>
      </select>
    </div>

    <h4 class="theme_option_headline_sub"><?php _e( 'フォントサイズ設定', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <input type="number" name="entire_options[fontsize]" value="<?= $options['fontsize']; ?>" size="50" step="1">
    </div>

    <h4 class="theme_option_headline_sub"><?php _e( '行間設定', 'colorful' ); ?></h4>
    <div class="theme_option_input">
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( 'メニュー設定', 'colorful' ); ?></h3>

    <h4 class="theme_option_headline_sub"><?php _e( 'メニュー表示', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
    ...
    <h4 class="theme_option_headline_sub"><?php _e( '【応用】メニュー位置', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
    ...
    <h4 class="theme_option_headline_sub"><?php _e( 'メニュー名', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <input type="text" name="entire_options[menu_name]" value="<?= $options['menu_name']; ?>" size="50">
    </div>

    <h4 class="theme_option_headline_sub"><?php _e( 'メニューデザイン', 'colorful' ); ?></h4>
    <div class="theme_option_input">
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '2カラム設定', 'colorful' ); ?></h3>
    <div class="theme_option_input">
      <?php
        $sidebar_array = array (
          'left' => __( '左サイドバー', 'colorful' ),
          'right' => __( '右サイドバー', 'colorful' )
        );
        $checked = '';
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( 'ヘッダー', 'colorful' ); ?></h3>
    <div class="theme_option_input">
      ...
      ?>
      <option value="-1"<?= ($header_content == -1) ? ' selected="selected"': ''; ?>><?php _e( 'ウィジットテンプレート', 'colorful' ); ?></option>
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( 'フッター', 'colorful' ); ?></h3>
    <div class="theme_option_input">
      ...
      ?>
      <option value="-1"<?= ($footer_content == -1) ? ' selected="selected"': ''; ?>><?php _e( 'ウィジットテンプレート', 'colorful' ); ?></option>
      <?php foreach( $hover_posts as $hover ) : ?>
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '固定ページ設定', 'colorful' ); ?></h3>
    <h4 class="theme_option_headline_sub"><?php _e( '全ての固定ページにも全体設定を反映させる', 'colorful' ); ?></h4>
    <div class="theme_option_input">
      <?php
        $all_array = array (
          'on' => 'ON',
  ...
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( 'ブログ一覧表示設定', 'colorful' ); ?></h3>
    <div class="theme_option_input">
      <select name="entire_options[blog_type]">
      <?php
        $blog_type_array = array (
          'design1' => __( 'デフォルト', 'colorful' ),
          'design2' => __( 'カード型', 'colorful' )
        );
        $checked = '';
...
?>
<div class="wrap">
<h2><?php _e( 'プログラムの追加', 'colorful' ); ?></h2>
<p><?php _e( 'プログラムを追加する場合は、こちらに記入することをオススメいたします。<br>
なお、こちらでのプログラム追加は、全ページに反映されます。', 'colorful' ); ?></p>
<form method="post" action="options.php">
<?php settings_fields( 'custom_css' ); do_settings_sections( 'custom_css' ); ?>
  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( 'CSSプログラムの追加', 'colorful' ); ?></h3>
    <textarea name="custom_css[css]" cols="70" rows="30"><?= esc_html($css['css']); ?></textarea>
  </div>

  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '「head内」にプログラムを追加する', 'colorful' ); ?></h3>
    <textarea name="custom_css[head]" cols="70" rows="30"><?= esc_html($css['head']); ?></textarea>
  </div>

  <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e( '「body内」にプログラムを追加する', 'colorful' ); ?></h3>

 ...
 */
 ...
 // ページ幅
 if( is_home() || is_single() || is_archive() || is_search() || (get_field('entire_enable', $post_id) === 'on' || $theme_options['all_enable'] === 'on') ) { // 全体設定
  ...
  $sp_enable = get_field('sp_enable', $post_id);
 }
?><!DOCTYPE html>
<html class="<?php echo $post_type; ?>" <?php language_attributes(); ?>>
<head>
...
<?php if( get_field('countdown_indesign', $post_id) === 'design3' ) : ?>
...
<?php endif; ?>
<?php if( get_field('countdown_indesign', $post_id) === 'design4' ) : ?>
...
<?php
 ...
 if( is_home() || is_single() || is_archive() || is_search() || (get_field('entire_enable', $post_id) === 'on' || $theme_options['all_enable'] === 'on') ) { // 全体設定
  ...
  if( get_field('sp_fontsize_default', $post_id) ) {
   $sp_fontsize_default = get_field('sp_fontsize_default', $post_id);
  } else {
   $sp_fontsize_default = 85;
  }
  // 行間
 ...
 // サイドバー背景色反映
 if( $entire_enable || get_field('post_layout', $post_id) != 1 ) { // 全体設定
...
<style type="text/css">
	...
	table[class^="fullwidth_tilt"] > tbody > tr > td {
		padding: 20px calc(50% - <?php echo intval(($pwidth - 100) / 2); ?>px);
	}
...
@media only screen and (max-width: 800px){
	#main {
		font-size: <?php echo $sp_fontsize_default; ?>% !important;
	}
}
</style>
<?php endif; ?>
...
<?php
	...
	if( get_field('shadow_enable', $post_id) === 'on' ) :
?>
<style type="text/css">
	.site {
		box-shadow: 0 0 10px <?php echo $shadow_color; ?>;
...
<?php if( get_field('sp_url', $post_id) != '' ) : ?>
<script type="text/javascript">
if ( ((navigator.userAgent.indexOf('iPhone') > 0 &&
	navigator.userAgent.indexOf('iPad') == -1) ||
	navigator.userAgent.indexOf('iPod') > 0 ||
	navigator.userAgent.indexOf('Android') > 0) ) {
	location.href = '<?php the_field('sp_url', $post_id); ?>';
}
</script>
<?php endif; ?>
</head>
...
<?php
if( is_home() || is_singular('post') || is_archive() || is_search() || (get_field('entire_enable', $post_id) === 'on' || $theme_options['all_enable'] === 'on') ) { // 全体設定
	...
	$menu_enable = get_field('menu_enable', $post_id);
	$menu_name = get_field('menu_name', $post_id);
	$menu_design = get_field('menu_design', $post_id);
...
<?php if( get_field('countdown_top', $post_id) === 'off' ) : ?>
<style type="text/css">
.navbar-fixed-top {
	height: 30px;
	position: fixed;
...
<div class="navbar navbar-inverse navbar-fixed-top" onMouseOver="dispWin();">
      ...
      <div class="brand">
	<!-- カウントダウンはここに追加 -->
    <p><?php echo get_field('countdown_name', $post_id); ?>　<span id="CDT"></span></p>
	<!-- カウントダウンはここまで -->
...
<?php if( is_home() || is_singular('post') || is_archive() || is_search() ) : ?>
	...
	<?php endif; ?>
<?php elseif( is_page() ) : ?>
	<?php if( (get_field('entire_hf_enable', $post_id) === 'on' || $theme_options['all_enable'] === 'on') && $theme_options['header_content'] > 0 ) : ?>
	<header id="header">
	<?php
	  $post = get_post($theme_options['header_content']);
	  setup_postdata($post);
	...
	<?php if( (get_field('entire_hf_enable', $post_id) === 'on' || $theme_options['all_enable'] === 'on') && $theme_options['header_content'] == -1 ) : ?>
	<header id="header" class="widget-template clear">
		<?php if ( is_active_sidebar( 'header-1' ) ) : ?><div class="widget-area"><?php dynamic_sidebar( 'header-1' ); ?></div><?php endif ?>
		<?php if ( is_active_sidebar( 'header-2' ) ) : ?><div class="widget-area"><?php dynamic_sidebar( 'header-2' ); ?></div><?php endif ?>
	</header><!-- #header -->
...
<?php if( get_field('topheader_image', $post_id) ) : ?>
<?php $image = wp_get_attachment_image_src( get_field('topheader_image', $post_id), 'full' ); ?>
